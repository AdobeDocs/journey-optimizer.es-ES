...
**En esta página:** Aprenda a etiquetar, agregar y personalizar fragmentos de contenido de Adobe Experience Manager en campañas y recorridos de Journey Optimizer, incluido el trabajo con variaciones y Experience Decisioning.
...
## Creación y asignación de una etiqueta en Experience Manager {#create-tag}
...
Asigne una etiqueta cuyo ID siga a `ajo-enabled:{AJO-OrgId}/{AJO-SandboxName}`, usando su ID de organización de Journey Optimizer y el nombre de la zona protegida en lugar de los marcadores de posición, por ejemplo, `ajo-enabled:123A12A123A123A12A@AdobeOrg/prod`.

Para crear la etiqueta en Experience Manager:

1. Vaya a **Herramientas** > **Etiquetado**.

   ![](assets/do-not-localize/aem_tag_1.png)

1. Cree una estructura de etiquetas anidada de modo que el ID de etiqueta completo coincida con el formato anterior:

   1. En el nivel raíz, cree una carpeta denominada `ajo-enabled`.

   1. En `ajo-enabled`, cree una etiqueta para el identificador de su organización, por ejemplo, `123A12A123A123A12A@AdobeOrg`.

   1. En esa etiqueta de organización, cree una etiqueta para la zona protegida, por ejemplo, `prod`.

   La ruta combinada genera un identificador de etiqueta como `ajo-enabled:123A12A123A123A12A@AdobeOrg/prod`.

1. Para aplicarlo a un fragmento de contenido, ábralo en el editor.

1. En **Propiedades**, agregue la etiqueta que creó.
...
>[!NOTE]
>
>Los fragmentos de contenido de AEM en Decisioning están disponibles para los clientes que integran Adobe Experience Manager **as a Cloud Service** o **Managed Services** con Adobe Journey Optimizer. [Más información](aem-fragments-gs.md)

### Requisitos previos y protecciones

* Los fragmentos de contenido deben etiquetarse en Adobe Experience Manager con la etiqueta `ajo-enabled:{OrgId}/{SandboxName}` antes de que aparezcan en el selector de decisiones. [Aprenda a crear y asignar una etiqueta](#create-tag)
* Solo están disponibles los fragmentos de contenido en un estado **publicado**.
* Puede agregar hasta **cinco** fragmentos de contenido de AEM a un solo elemento de decisión.

### Aprovechamiento de fragmentos de contenido de AEM en Decisioning

Una vez creado y publicado el fragmento de contenido de AEM, debe hacer lo siguiente:

1. Vincularlo a un elemento de decisión seleccionándolo en los atributos del elemento de decisión.
1. Aproveche esta opción en una política de decisiones para que el contenido adecuado aparezca al cliente correcto.

>[!NOTE]
>
>Los fragmentos de contenido de AEM en Decisioning son compatibles con **Adobe Experience Manager as a Cloud Service** y **Adobe Experience Manager Managed Services**. [Más información](aem-fragments-gs.md)

➡️ [Asociar un fragmento de contenido de AEM a un elemento de decisión](../experience-decisioning/items.md#aem-fragments)

➡️ [Aprovechar fragmentos de contenido de AEM en una directiva de decisión](../experience-decisioning/fragments-decision-policies.md#aem-fragments-decisioning)
