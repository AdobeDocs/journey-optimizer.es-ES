...
**En esta página:** Aprenda a integrar un proveedor de mensajería personalizado en Adobe Journey Optimizer creando credenciales de API, eligiendo un método de autenticación y configurando encabezados, cargas útiles y configuraciones de entrada para enviar mensajes SMS y RCS.
...
## Crear credenciales de API {#api-credential}

>[!CONTEXTUALHELP]
>id="ajo_admin_sms_api_byop_channel_type"
>title="Tipo de canal"
>abstract="Opcional. Clasifique los mensajes enviados con esta credencial del proveedor de SMS personalizada, por ejemplo, SMS o RCS. Journey Optimizer escribe el valor en los eventos de experiencia XDM para que pueda crear informes y rastrear el envío por canal."

>[!CONTEXTUALHELP]
>id="ajo_admin_sms_webhook_require_auth"
>title="Autenticación"
>abstract="Cuando se habilita, solo se aceptan solicitudes autenticadas a través de Adobe IMS. Los clientes deben incluir un token de OAuth válido al enviar datos a este punto final."
...
1. Habilite la opción **[!UICONTROL compatibilidad con mTLS]**, que garantiza que tanto el cliente como el servidor se autenticarán mutuamente antes de establecer una conexión segura.
   ...
   Tenga en cuenta que mTLS solo se aplica al extremo del proveedor de SMS (envío de mensajes). El extremo del token de OAuth no debe utilizar mTLS. Asegúrese de que mTLS esté deshabilitado en el extremo del token antes de realizar la prueba.

   >[!IMPORTANT]
   >
   >Configure el extremo de envío de SMS para que confíe en la cadena de autoridad de certificación de Adobe Experience Platform descargando el certificado público de la API de certificado público [MTLS](https://experienceleague.adobe.com/es/docs/experience-platform/data-governance/mtls-api/public-certificate-endpoint) y agregándolo al almacén de confianza del servidor (CN de cliente esperado: `ajo-sms.aep-mtls.adobe.com`); de lo contrario, Journey Optimizer omite el certificado de cliente y falla el envío de SMS.
...
Journey Optimizer actualiza los tokens de OAuth de forma dinámica al expirar el conector de SMS personalizado.
