...
title: CC (copia de carbono) en la configuración del canal de correo electrónico
description: Configuración de destinatarios CC visibles en el canal de correo electrónico de Journey Optimizer. Aprenda a configurar el campo CC, en qué se diferencia de CCO, y las limitaciones.
...
**En esta página:** Aprenda a agregar un campo CC visible a los correos electrónicos en una configuración de canal de Adobe Journey Optimizer, en qué se diferencia de CCO, y las recomendaciones y limitaciones relacionadas.
...
Puede especificar cualquier dirección externa en el formato correcto, excepto una dirección de correo electrónico definida en un subdominio delegado a Adobe. Por ejemplo, si delegó el subdominio *marketing.luma.com* a Adobe, cualquier dirección como *abc@marketing.luma.com* está prohibida.

>[!CAUTION]
>
>Solo puede definir una dirección de correo electrónico. Asegúrese de que la dirección CC tenga suficiente capacidad de recepción para almacenar todos los correos electrónicos enviados con la configuración de canal actual.
>
>Hay más recomendaciones en [esta sección](#cc-recommendations-limitations).
...
* Un **atributo contextual** - este valor **solo se puede usar en campañas activadas por API**. Se recupera de la carga útil de API que debe incluir la variable de contexto `context.channel.email.ccvalues` con el valor de dirección CC pasado como cadena.
  ...
  >[!WARNING]
  >
  >Cuando CC se establece usando una **variable de contexto**, solo se admite en **campañas activadas por API**. Si utiliza esa configuración de canal en un recorrido o una campaña de acción, el correo electrónico se envía únicamente al destinatario principal; no se envía ninguna copia a la dirección CC.

Cualquier [archivo adjunto](../email/pdf-attachments.md) incluido en el mensaje se envía tanto al destinatario principal como a la dirección de CC.

Si el valor CC no es válido o falta en el momento del envío (por ejemplo, una variable de contexto vacía), la copia CC se omite; el destinatario principal sigue recibiendo el correo electrónico.
...
* **Administración de la bandeja de entrada:** La bandeja de entrada utilizada para CC debe administrarse correctamente para el espacio y la entrega. Si la bandeja de entrada devuelve devoluciones, es posible que algunos correos electrónicos no se reciban.
