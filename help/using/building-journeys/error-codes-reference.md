...
description: Obtenga información acerca de los códigos de error comunes en  [!DNL Adobe Journey Optimizer]  y cómo solucionarlos
...
**En esta página:** Aprenda cómo están estructurados los códigos de error de Adobe Journey Optimizer, dónde encontrarlos y cómo solucionar errores comunes en recorridos, campañas y configuraciones de mensajes.

>[!ENDSHADEBOX]

[!DNL Adobe Journey Optimizer] usa códigos de error estandarizados para ayudarle a identificar y resolver problemas rápidamente en recorridos, campañas y configuraciones de mensajes. Comprender estos códigos de error puede reducir significativamente el tiempo de solución de problemas y ayudarle a mantener un rendimiento óptimo de la campaña.

## Explicación de la estructura de código de error {#error-code-structure}

Los códigos de error [!DNL Adobe Journey Optimizer] siguen un patrón de nomenclatura coherente que ayuda a identificar el componente y el tipo de problema:

* **Prefijo de servicio**: indica qué servicio [!DNL Adobe Journey Optimizer] generó el error.
Ejemplos: CJMPTS (servicio push/transporte), CJMRT (tiempo de ejecución del Recorrido), CJMMAS (servicio de creación de mensajes), CJMCMP (campaña), CJMTL (capa de transporte), CJMRPS (servicio de creación de informes/aprovisionamiento)
...
Los códigos de error aparecen en varias ubicaciones dentro de [!DNL Adobe Journey Optimizer]:
...
| **CJMPTS-1510-500** | Error interno del servidor en el envío por canal push | Fallo de push/transporte back-end; error de proveedor o infraestructura | &#x200B;1. Compruebe la configuración de aprovisionamiento de canal<br/>2. Compruebe que las credenciales de inserción son válidas<br/>3. Vuelva a intentar la operación<br/>4. Si es persistente, póngase en contacto con el soporte técnico de Adobe con el ID de solicitud <br/><br/>**Documentación relacionada**: [Configuración push](../push/push-configuration.md) |
...
| **CJMCMP-2003-400** | Titular de la interfaz de usuario: &quot;El experimento es incorrecto&quot; en el correo electrónico de Designer | Experimento o proveedor de datos obsoleto o ausente; limpieza del experimento fallida, discrepancia de esquema o error de validación de la interfaz de usuario | &#x200B;1. Quitar campos de experimento no utilizados<br/>2. Validar conexiones de esquema y proveedor de datos<br/>3. Vuelva a cargar la interfaz de usuario y borre la caché del explorador<br/>4. Volver a crear nodo/correo electrónico si el problema no se ha resuelto <br/><br/>**Documentación relacionada**: [Experimentos de contenido](../content-management/content-experiment.md) |
...
| Código de error | Descripción | Causa principal | Resolución |
|------------|-------------|-----------|-----------|
| **CJMTL-010018-422** | &quot;No se permite Personalization en el nombre de dominio&quot; al guardar o enviar contenido | La validación demasiado estricta interrumpió temporalmente la personalización del dominio href dinámico | &#x200B;1. Refactorizar vínculos si se usan variables de dominio<br/>2. Compruebe que la última versión de AJO esté en uso<br/>3. Vuelva a intentar la operación<br/>4. Use dominios estáticos si el problema persiste <br/><br/>**Documentación relacionada**: [Sintaxis de Personalization](../personalization/personalization-syntax.md), [Diseño de correo electrónico](../email/content-from-scratch.md) |
| **CJMTL-010011-422** | Entidad no procesable: el envío push/SMS/correo electrónico falla, dice &quot;campo no válido&quot; | Faltan datos de carga útil o de destinatario/contacto o no son válidos | &#x200B;1. Inspeccionar registros para detectar errores de campo específicos<br/>2. Se corrigió la información de perfil/contacto<br/>3. Validar con perfil de prueba<br/>4. Refactorice el formato de carga útil según sea necesario <br/><br/>**Documentación relacionada**: [Administración de perfiles](../audience/get-started-profiles.md), [Perfiles de prueba](../audience/creating-test-profiles.md) |
...
| Código de error | Descripción | Causa principal | Resolución |
|------------|-------------|-----------|-----------|
| **CJMRPS-1047-409** | &quot;Conflicto. El conjunto de datos ya se ha agregado al agregar un conjunto de datos de informes | Intentando agregar un conjunto de datos que ya está aprovisionado | &#x200B;1. Revise la configuración del conjunto de datos en la configuración de informes<br/>2. No vuelva a agregar los conjuntos de datos ya presentes<br/>3. Usar listas de comprobación de migración oficiales para informar sobre la migración<br/>4. Eliminar referencias duplicadas de conjuntos de datos <br/><br/>**Documentación relacionada**: [Información general de informes](../reports/gs-reports.md), [Informes de campañas](../reports/campaign-global-report-cja.md), [Informes de Recorridos](../reports/journey-global-report-cja.md) |
...
Utilice estas prácticas para reducir los errores evitables y mejorar la fiabilidad.
...
* [Desmitificación de  [!DNL Adobe Journey Optimizer] códigos de error: Parte 2](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer-blogs/demystifying-adobe-journey-optimizer-error-codes-root-causes-and/bc-p/782661?profile.language=es){target="_blank"}
...
* **TL;DR:** Esta página es una guía de referencia para códigos de error de Adobe Journey Optimizer estandarizados organizados por prefijo de servicio, que explica la causa raíz de cada error y proporciona instrucciones de resolución paso a paso.

**Intenciones:**

* Identifique qué servicio de AJO generó un error utilizando el prefijo de servicio del código de error
* Diagnosticar y resolver los errores de inserción y transporte (CJMPTS) que afectan al envío de mensajes
* Solucionar errores de tiempo de ejecución y API de recorrido (CJMRT) durante la ejecución del recorrido o el procesamiento de eventos
* Corregir errores de creación de mensajes (CJMMAS) al crear, guardar o publicar mensajes
* Resolver errores de campaña (CJMCMP) durante la activación o aprobación de la campaña
* Enviar errores persistentes al Soporte de Adobe con la información correcta

**Glosario:**

* **Prefijo de servicio**: El código alfanumérico al principio de un código de error de AJO que identifica qué servicio generó el error (por ejemplo, CJMRT = Tiempo de ejecución de Recorrido) *(específico del producto)*
* **Código de estado HTTP**: El código de estado estándar incrustado en un código de error de AJO (por ejemplo, 400 = Solicitud incorrecta, 403 = Prohibida, 422 = Entidad no procesable, 500 = Error interno del servidor)
* **ID de solicitud**: Identificador único que acompaña a un error que es necesario al escalar al soporte técnico de Adobe *(específico del producto)*
* **CJMRT**: prefijo de servicio de tiempo de ejecución de Recorrido: errores durante la ejecución de recorrido y operaciones de API *(específico del producto)*
* **CJMMAS**: prefijo del servicio de creación de mensajes: errores durante la creación y publicación del mensaje *(específico del producto)*
* **CJMPTS**: prefijo del servicio push/transporte: errores durante la notificación push y el transporte de mensajes *(específico del producto)*

**Protecciones:**

* Las variantes de correo electrónico deben incluir un vínculo de exclusión/cancelación de suscripción, omitiendo los déclencheur CJMMAS-2001-200.
* La detención de un recorrido requiere el permiso Administrar recorridos (relevante para los errores de CJMRT que implican permisos).
* La propagación de DNS para la delegación de subdominios puede tardar hasta 72 horas (según CJMRT-080608-400).
* Las claves de búsqueda para las actividades de búsqueda de conjuntos de datos deben definirse en modo avanzado, no en modo simple.

**Terminología:**

* Nombre canónico: Código de error — Acrónimo: n/a — variants: error message, error identifier
* Sinónimos: &quot;prefijo de servicio&quot; = &quot;prefijo de error&quot; = &quot;identificador de componente&quot;
* No confunda: &quot;400 Solicitud incorrecta&quot; ≠ &quot;422 Entidad no procesable&quot;: 400 indica una entrada con formato incorrecto, 422 indica un formato válido pero un contenido no válido por reglas de esquema

**PREGUNTAS MÁS FRECUENTES:**

* **Q: ¿Cómo sé qué servicio de AJO causó un error?** — Lea el prefijo del servicio al principio del código de error: CJMPTS (push/transport), CJMRT (tiempo de ejecución del recorrido), CJMMAS (creación de mensajes), CJMCMP (campaña), CJMTL (capa de transporte), CJMRPS (creación de informes/aprovisionamiento).
* **Q: ¿Qué debo hacer cuando recibo un error de la serie 500?** — Vuelva a intentarlo pasados unos minutos, compruebe si hay interrupciones en Adobe Status y, a continuación, diríjase al servicio de asistencia de Adobe con el código de error completo y el ID de solicitud si el problema persiste.
* **Q: ¿Por qué CJMMAS-2001-200 muestra un banner de error aunque el estado sea &quot;correcto&quot;?** — Falta un vínculo obligatorio de exclusión/cancelación de suscripción en una variante de correo electrónico; añádalo a todas las variantes y versiones de idiomas.
* **Q: ¿Qué información debo recopilar antes de ponerme en contacto con el Soporte técnico de Adobe?** — Recopile el código de error completo, el ID de solicitud, las marcas de tiempo, los pasos que se seguirán para reproducir y cualquier detalle de configuración pertinente.
* **Q: ¿Qué causa CJMRT-030012-422?** — Datos de entrada no válidos, como una referencia a una audiencia, evento o atributo inexistente; compruebe que todos los objetos a los que se hace referencia existen y están activos.
