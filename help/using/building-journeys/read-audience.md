...
>abstract="Añade todos los perfiles cualificados de un público de [!DNL Adobe Experience Platform] seleccionado a este recorrido. Se ejecuta una vez o según una programación."
...
**Requisitos previos:** Una audiencia de [!DNL Adobe Experience Platform] que se ha creado y evaluado (estado Realized), un área de nombres de identidad basada en personas seleccionada para el recorrido y, para ejecuciones recurrentes, una comprensión de [los límites de programación y rendimiento](../start/guardrails.md#read-segment-g).
...
>title="Público"
>abstract="El público [!DNL Adobe Experience Platform] cuyos perfiles entrarán en este recorrido. Todos los perfiles cualificados se leen. Se recomiendan los públicos por lotes para obtener recuentos fiables y coherentes, y solo se puede leer un público por actividad."
...
Todas las protecciones y limitaciones de la actividad **Leer audiencia** (concurrencia, rendimiento, una audiencia por actividad, tiempo de espera del trabajo, reintentos y más) se enumeran en [Protecciones y limitaciones](../start/guardrails.md#read-segment-g).
...
>abstract="La fecha y la hora en que el recorrido empieza a leer el público y los perfiles comienzan a entrar. Combínelo con las opciones de periodicidad siguientes para programar ejecuciones recurrentes."

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_scheduler_repeat_until"
>title="Repetir hasta"
>abstract="La fecha en la que se detienen las ejecuciones recurrentes. Después de esta fecha, el recorrido ya no lee el público ni admite nuevos perfiles."

>[!CONTEXTUALHELP]
>id="ajo_journey_read_segment_scheduler_repeat_every"
>title="Repetir cada"
>abstract="La frecuencia con la que el recorrido vuelve a leer el público y se ejecuta de nuevo, por ejemplo, diaria o semanalmente. Determina el intervalo de periodicidad entre ejecuciones hasta que se alcance la fecha Repetir hasta."
...
>abstract="Borra todos los participantes del recorrido antes de que se lea cada público nuevo, de modo que cada ejecución se inicia de nuevo y los perfiles pueden volver a entrar en cada ocurrencia."
...
## Segmentación de audiencia en recorrido {#audience-targeting-in-journeys}
...
* **`inAudience()`tiempo de condición:** Al usar `inAudience()` en un nodo de condición dentro de un recorrido de audiencia de lectura, se lee la pertenencia al segmento de la proyección por lotes del perfil. Los datos de esta proyección se actualizarán dentro de las **2 horas** posteriores a la ingesta. Para obtener detalles completos sobre los escenarios de tiempo de propagación, consulte la [documentación de la función inAudience](functions/functioninaudience.md#propagation-timing).
...
Si el recuento no coincide o las ejecuciones de perfil cero persisten después de seguir los pasos anteriores, póngase en contacto con el soporte de Adobe. Tenga listo: nombre/ID de audiencia, nombre/ID de recorrido, tiempo(s) de ejecución programado, zona protegida y una breve descripción de la discrepancia (por ejemplo, &quot;La audiencia muestra 10 000 realizados, solo 2 000 ingresaron al recorrido el [date]&quot;).
...
Para obtener la lista completa de protecciones de audiencia de lectura (incluidos los límites de reintento y rendimiento), consulte [Protecciones y limitaciones](../start/guardrails.md#read-segment-g).
