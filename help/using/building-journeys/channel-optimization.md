...
## Establecer el método de optimización del canal {#optimization-modes}

>[!CONTEXTUALHELP]
>id="ajo_channel_optimization_method"
>title="Definir el funcionamiento de la selección de canal"
>abstract="Elija la forma en que Journey Optimizer selecciona el mejor canal para cada cliente: **Prioridad manual**: los canales se prueban en el orden definido. La disponibilidad se determina aplicando las preferencias de suscripción y las reglas de consentimiento de marketing asociadas a las configuraciones de canal seleccionadas y todas las reglas empresariales (por ejemplo, restricción de frecuencia de canal) asociadas a la campaña o el recorrido. **Atributo del perfil de cliente**: el canal que coincide con la preferencia declarada del cliente en su perfil se selecciona primero. Si no se encuentra ninguna preferencia, se aplica la prioridad manual. **Optimizado con IA**: un modelo de aprendizaje automático puntúa cada canal según la participación histórica del cliente, y se selecciona el canal disponible con mayor puntuación."
