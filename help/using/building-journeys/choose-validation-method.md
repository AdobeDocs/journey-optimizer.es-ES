...
description: Compare la simulación de Recorrido, el modo de prueba y la ejecución en seco del Recorrido y elija el método de validación adecuado para el recorrido antes de publicar.
...
**En esta página:** Compare la simulación de Recorrido, el modo de prueba y la ejecución en seco de Recorrido, y descubra cuál se ajusta a su etapa actual de construcción de un recorrido, desde una iteración rápida durante el diseño hasta una comprobación final previa al lanzamiento en comparación con su audiencia en vivo.
...
* **Todavía estoy diseñando mi recorrido y quiero probar una rama rápidamente, sin crear perfiles de prueba.** → Usar **[Simulación De Recorrido](simulate-journey-gs.md)**.
* **Quiero recorrer manualmente mi recorrido de borrador paso a paso, usando perfiles reales (pero designados como prueba).** → Usar **[modo de prueba](testing-the-journey.md)**.
* **Estoy a punto de publicar y quiero una comprobación final de mi audiencia de producción real, sin ponerme en contacto con nadie.** → Usar **[Recorrido en seco](journey-dry-run.md)**.
...
**Cuándo se debe usar:** Iteración rápida durante el diseño del recorrido, especialmente justo antes de un plazo o al probar nuevas ramas de directivas de decisión.

[Simulación de Recorrido](simulate-journey-gs.md) valida su recorrido con usuarios simulados temporales, creados manualmente o autogenerados, sin necesidad de crear o esperar a que se propaguen los perfiles de prueba reales de Adobe Experience Platform (AEP). El agente de simulación de Recorrido genera automáticamente los eventos de prueba que necesita el recorrido y los relaciona con los usuarios simulados adecuados, lo que activa el recorrido en segundos.

Mecánica clave:

* Los usuarios simulados son temporales y desechables, no son perfiles reales en AEP.
...
[El modo de prueba](testing-the-journey.md) le permite validar un recorrido de borrador usando [perfiles de prueba de AEP](../audience/creating-test-profiles.md) persistentes. Los eventos se activan manualmente desde la interfaz para confirmar que la lógica de ramificación y la mecánica de envío de mensajes funcionan según lo diseñado, antes de que cualquier audiencia de producción toque el recorrido.

Mecánica clave:

* Solo los perfiles marcados como &quot;perfiles de prueba&quot; en el Perfil del cliente en tiempo real pueden introducir un recorrido en el modo de prueba.
* El modo de prueba solo está disponible para recorridos de borrador que usen un [área de nombres](../audience/get-started-identity.md), ya que debe comprobar en AEP si una persona es un perfil de prueba.
...
* Al deshabilitar el modo de prueba, se eliminan todos los perfiles que ingresaron al recorrido y se borran los informes.

>[!IMPORTANT]
>
>El modo de prueba envía mensajes reales a las bandejas de entrada reales de los perfiles de prueba, utilizando la misma canalización de entrega que la producción. No se pone en contacto con clientes reales, pero tampoco es una simulación &quot;en seco&quot;: asegúrese de que los perfiles de prueba utilizan direcciones que controla.

**Problema:** La creación y propagación de nuevos perfiles de prueba de AEP lleva tiempo, lo que limita cuando necesita validar rápidamente una nueva rama. Si ese es su objetivo, [Simulación de Recorrido](simulate-journey-gs.md) resuelve este problema específico en su lugar.
...
La respuesta normalmente se reduce a una pregunta: *¿qué tan cerca de la producción necesita que esté esta prueba?*

Si todavía **está iterando en el diseño de recorrido** — probando una rama nueva, comparando con un plazo — use **Simulación de Recorrido**. No necesita perfiles reales y se ejecuta en segundos. Recuerde que envía mensajes reales a las direcciones de ejecución configuradas en los usuarios simulados.

Si necesita **comprobar manualmente la rama y la lógica de mensaje paso a paso** y está dispuesto a crear o reutilizar perfiles de prueba de AEP, use **Modo de prueba**. Solo recuerde que envía mensajes reales a las bandejas de entrada reales de esos perfiles de prueba.

Si está a punto de **publicar** y desea una comprobación final de la audiencia de producción real, sin ponerse en contacto con nadie ni cambiar los datos del perfil, use **Ejecución en seco del Recorrido**.

>[!TIP]
>
>**¿No está seguro de por dónde empezar?** La mayoría de los equipos usa **Simulación de Recorrido** al generar y luego **Ejecución en seco de Recorrido** justo antes de publicar. Alcance **Modo de prueba** cuando necesite recorrer manualmente la lógica de rama con perfiles de prueba reales en lugar de los simulados.

## Comparación rápida {#quick-comparison}

| Método | Datos utilizados | ¿Envía mensajes reales? | Mejor para |
|---|---|---|---|
| [Simulación de Recorrido](simulate-journey-gs.md) | Usuarios simulados temporales, creados manualmente o autogenerados | Sí: a las direcciones de ejecución configuradas en los usuarios simulados | Iteración rápida, especialmente para las nuevas ramas de política de decisión, sin esperar a la propagación real del perfil de prueba |
| [Modo de prueba](testing-the-journey.md) | Perfiles de prueba de AEP persistentes | Sí: a las bandejas de entrada reales de los perfiles de prueba, mediante la canalización de entrega de producción | Verificación manual de la lógica de bifurcación/mensaje paso a paso en un borrador de recorrido |
...
Ninguno de estos métodos contacta con clientes reales. Los datos de perfil tampoco se tocan en todos los casos, excepto en que el modo de prueba actualiza los perfiles de prueba utilizados para ejecutarlos (no los perfiles reales del cliente).
...
* **Creando perfiles de prueba de AEP cuando la simulación de Recorrido funcionaba.** Si solo necesita validar una nueva rama o ruta de política de decisión rápidamente, la simulación omite la espera de la propagación del perfil de prueba por completo: guarde el modo de prueba para cuando realmente necesite perfiles de prueba reales.
* **Tratando el modo de prueba como &quot;seco&quot;.** Los perfiles de modo de prueba reciben mensajes reales a través de la canalización de entrega de producción. Asegúrese de que los perfiles de prueba solo utilicen las direcciones que controla.
* **Se espera que la ejecución en seco del Recorrido detecte problemas de contenido o envío.** La ejecución en seco omite los nodos de acción por completo: valida el alcance de la audiencia y la lógica de rama, no el contenido del mensaje ni la mecánica de envío. Para ello, utilice el modo de simulación o de prueba.
* **Olvidando el requisito del área de nombres para el modo de prueba.** El modo de prueba solo funciona en recorridos de borrador que utilizan un área de nombres; sin una, Journey Optimizer no puede comprobar si un perfil está marcado como perfil de prueba.

## Próximos pasos {#next-steps}

* **[Introducción a la simulación de recorrido](simulate-journey-gs.md)** — Ejecute su primera simulación
* **[Probar el recorrido](testing-the-journey.md)**: active el modo de prueba con perfiles de prueba de AEP
...
* **TL;DR:** Esta página compara los tres métodos de validación de recorrido de Adobe Journey Optimizer (simulación de Recorrido, modo de prueba y ejecución en seco de Recorrido) y proporciona una selección rápida de una pregunta, una guía de decisión, una tabla de comparación rápida y una lista de errores comunes para ayudar a los usuarios a elegir el correcto para su etapa actual de creación de un recorrido.

**Intenciones:**

* Elija el método de validación correcto para una fase determinada de la creación de recorridos
* Comparar Simulación de Recorrido, Modo de prueba y Ejecución en seco de Recorrido en paralelo
* Comprenda cuándo utilizar la simulación de Recorrido para una iteración rápida sin perfiles de prueba reales
* Comprenda cuándo utilizar el modo de prueba para la validación manual paso a paso con perfiles de prueba reales
...
* **Modo de prueba**: método de validación que utiliza perfiles de prueba AEP persistentes, marcados en Perfil del cliente en tiempo real, para recorrer manualmente la rama y la lógica de mensaje de un recorrido de borrador. *(específico del producto)*
...
* El modo de prueba solo está disponible para recorridos de borrador que utilicen un área de nombres y admite un máximo de 100 perfiles de prueba por sesión
* Los eventos del modo de prueba solo se pueden activar desde la interfaz, no desde sistemas externos a través de la API
* El modo de prueba envía mensajes reales a las bandejas de entrada reales de los perfiles de prueba mediante la canalización de entrega de producción
* Al deshabilitar el modo de prueba, se eliminan todos los perfiles que ingresaron al recorrido y se borran sus informes
...
* **Q: ¿El modo de prueba envía correos electrónicos o SMS reales?** — Sí. El modo de prueba envía mensajes reales a las bandejas de entrada reales de los perfiles de prueba, utilizando la misma canalización de entrega que la producción. No contacta con clientes reales, pero los mensajes en sí son reales.
...
* **Q: necesito validar una nueva rama rápidamente antes de la fecha límite. ¿Qué método encaja?** — Simulación de Recorrido; genera usuarios simulados desechables bajo demanda en lugar de requerir la creación previa y la espera de perfiles de prueba reales.
...
* **Q: ¿Puedo activar eventos de modo de prueba desde un sistema externo?** — No; en el modo de prueba, los eventos solo se pueden activar desde la interfaz, no desde sistemas externos a través de la API.
