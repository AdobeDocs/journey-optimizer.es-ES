...
El nuevo proceso de creación de mensajes en Adobe Journey Optimizer se describe en [página](../rn/inline-messages.md). Se realizará una conversión automática de recorridos. Dicho esto, necesitaremos su ayuda con algunos pasos.
...
1. Busque errores en la migración de la versión de borrador. [Más información](../rn/inline-messages-steps.md#migration-step-2-5)
...
Para encontrar estos recorridos, vaya a la **Recorridos** y filtre la lista en &quot;Estado = Activo&quot; y &quot;Tipo = Leer segmento&quot;. También puede realizar pedidos cronológicamente desde la fecha más temprana hasta la más reciente de &quot;Publicado&quot;.
...
* Compruebe que no sean recorridos recurrentes. No son ad hoc. Lo más probable es que quieras mantenerlos en vivo. Por ejemplo, este es un recorrido recurrente (no ad-hoc):

   ![](assets/inline-migration-steps2.png)

* Si ha utilizado oyentes de espera o eventos en esos recorridos, es posible que los perfiles sigan estando dentro. Observe la fecha de ejecución del recorrido y añada cualquier hora o día que haya definido en sus esperas o oyentes de eventos para deducir la fecha real en la que no quedan perfiles dentro. Si esa fecha es anterior, puede detener el recorrido. De lo contrario, este recorrido se moverá automáticamente al estado &quot;Finalizado&quot; 30 días después de la fecha de ejecución del recorrido.
...
* Si tiene mensajes que no se utilizan en recorridos y que desea conservar, guárdelos como plantillas. Tenga en cuenta que aún podrá acceder a ellas hasta que desaparezca.
...
Para la fase automatizada, consulte esta [página](../rn/inline-messages.md#process).

Para la fase manual, estas son las acciones que se deben realizar en la variable **entorno limitado de producción**:
...
Compruebe si hay errores en los recorridos activos migrados automáticamente en el informe de estado.
...
Publique la nueva versión del recorrido. La versión anterior en directo pasará al estado &quot;Cerrado&quot;.
...
### 5. Observe los errores en la migración de la versión de borrador{#migration-step-2-5}

Haga clic en el **Comprobar estado** en el banner superior y compruebe que no se ha producido ningún error durante la migración automática y que no queda nada por migrar. Tenga en cuenta que cualquier recorrido de error (con mensajes) quedará obsoleto después del 5 de septiembre (en todos los entornos limitados).
...
La segunda iteración tendrá lugar por la noche entre el 1 de agosto y el 2 de agosto.
...
Si no ha publicado versiones de recorrido más recientes (consulte esta [sección](../rn/inline-messages-steps.md#migration-step-2-3)) en el tiempo que significa antes de la iteración 2 (1 de agosto), luego publique la versión más reciente y **detenga la versión anterior o la perderá** y sus informes asociados.
