...
Delegar subdominios de correo electrónico es un paso central en la [configuración de canal](../using/configuration/get-started-configuration.md); es necesario para poder enviar correos electrónicos desde Journey Optimizer. Los subdominios le permiten aislar los tipos de tráfico (por ejemplo, de marketing frente al transaccional), proteger la reputación de su dominio principal y acelerar el [calentamiento de IP](../using/configuration/ip-warmup-gs.md). Funcionan junto con la [configuración de canal de correo electrónico](../using/email/get-started-email-config.md) y la [monitorización de la entregabilidad](../using/reports/deliverability.md) para garantizar que los mensajes lleguen a las bandejas de entrada.

Puede elegir entre varios métodos de configuración: **delegación completa** (Adobe administra DNS), **Configuración de CNAME** o **delegación personalizada** (los certificados y DNS son de su propiedad). Si empieza con CNAME, más adelante [puede migrar a la delegación personalizada](../using/configuration/custom-subdomain-migration.md) para una seguridad más estricta. En esta sección también se tratan los registros de DMARC y PTR, los registros TXT de Google para Gmail y los grupos de IP. Para obtener instrucciones más amplias sobre la entregabilidad, consulte [Introducción a la entregabilidad](../using/reports/deliverability.md) y [Monitorizar las direcciones de correo electrónico](monitor-reputation-landing-page.md).
...
Migrar de CNAME a la delegación personalizada

Migre los subdominios configurados con CNAME existentes a la delegación personalizada para cumplir con las políticas de seguridad y obtener un control total sobre los certificados.

[Migrar el subdominio](../using/configuration/custom-subdomain-migration.md)
:::
...
- **[Configurar subdominios de la página de destino](../using/landing-pages/lp-subdomains.md)**: configure subdominios para páginas de destino y formularios de suscripción.
- **[Configurar subdominios web](../using/web/web-delegated-subdomains.md)**: delegue subdominios para experiencias web y el seguimiento.
